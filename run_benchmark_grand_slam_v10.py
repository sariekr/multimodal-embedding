...
SAMPLE_SIZE = 1000  # Increased for statistical significance
...
def compute_metrics(scores):
    ...
    for k in [1, 5, 10]:
        ...
        correct = 0
        for i in range(n):
            # indices of top k scores for query i
    ...
    mrr_sum = 0
    for i in range(n):
        # Rank of the correct image (i) among all images
        # argsort descending, find where index is i
...
def run_retrieval_benchmark(model, processor, model_info, dataset, dataset_name, text_col="caption", image_col="image"):
    ...
    try:
        ...
        metrics[f"{dataset_name} Time(s)"] = f"{inference_time:.1f}"
...
def run_winoground_full(model, processor, model_info):
    ...
    try:
        for example in dataset:
            ...
            s = s.float().cpu()
            # Logic: s[0,0]=T0I0, s[0,1]=T0I1, s[1,0]=T1I0, s[1,1]=T1I1
            
            # Text Score: Given image, correct caption?
            # I0: T0>T1? AND I1: T1>T0?
            t_correct = (s[0,0] > s[1,0]) and (s[1,1] > s[0,1])
            
            # Image Score: Given text, correct image?
            # T0: I0>I1? AND T1: I1>I0?
...
if __name__ == "__main__":
    ...
    # DocVQA
    try: ds_doc = load_dataset("nielsr/docvqa_1200_examples", split="test") # Use full set ~1200
    except: ds_doc = []
    
    # COCO (New) - Using a reliable subset if possible, or streaming
    try: 
        print("Loading COCO (1k sample)...")
        # Using a known parquet subset or streaming from main
        # 'merve/coco2017' is parquet. Validation split is 5k. We take 1k.
        ds_coco = load_dataset("merve/coco2017", split="validation").select(range(SAMPLE_SIZE))
    except: 
        print("COCO failed, skipping.")
        ds_coco = []
    ...
    for m_info in MODELS_TO_TEST:
        ...
        if len(ds_coco) > 0: 
            # COCO col is usually 'captions' (list)
    ...
    print("\n" + "="*100)
    print("GRAND SLAM BENCHMARK RESULTS (V10 - PROFESSIONAL)")
    print("="*100)
    ...
    if len(results) > 0:
        ...
        order = ["Model", "Flickr R@1", "Flickr MRR", "COCO R@1", "COCO MRR", "DocVQA R@1", "DocVQA MRR", "Wino Group"]
        # Add remaining keys
        ...
        # Save
        pd.DataFrame(results).to_csv("benchmark_v10_results.csv", index=False)
