"""
V26 (FIXED & VALIDATED) - MS-COCO (Full 5k) & Winoground

CHANGES from V25:
1. 🐛 CRITICAL FIX: Defined missing global SEED constant.
2. 🐛 CRITICAL FIX: Removed broken `valid_indices` passing. Now iterates dataset and checks cache directly.
   - Prevents misalignment when shuffling.
3. ⚡ OPTIMIZATION: Runs Winoground ONLY ONCE per model (dataset is fixed).
4. 🔍 VALIDATION: Checks results against reference ranges and warns if out-of-bounds.
5. 🔥 WARM-UP: Removed silent failure to expose errors.
6. 📝 TYPES: Added better type hints.

Usage:
  python run_benchmark_grand_slam_v26_fixed.py --runs 3 --batch-size 32
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("benchmark_v26.log"),
...
# --- ARGS ---
def parse_args():
    parser = argparse.ArgumentParser(description="Grand Slam Multimodal Benchmark V26")
    ...
    parser.add_argument("--output", type=str, default="benchmark_v26_results.csv", help="Output CSV file")
...
def prepare_dataset_cache(ds: Dataset) -> None:
    """Ensures all images in the dataset are downloaded and valid."""
...
def run_benchmark_coco(model: Union[PreTrainedModel, Any], 
    ...
    # Iterate dataset directly - rely on cache
    for _, item in enumerate(dataset):
        img = load_cached_image(item)
        if img is None:
            # Skip if missing (handled by prepare_dataset_cache ensuring >99%)
            continue
...
if __name__ == "__main__":
    ...
    for m_info in MODELS_TO_TEST:
        ...
        # 1. Winoground (Once per model)
        clean_memory()
        wino_metrics = {}
        try:
             # Load model temporarily for Wino
            ...
            clean_memory()
             ...
             logger.error(f"Winoground Failed: {e}")
        ...
        for run_idx in range(ARGS.runs):
            ...
            if ARGS.sample_size < len(ds_full):
                # Note: indices inside ds_run are re-mapped [0..N]
            ...
            try:
                ...
                # Merge Wino (static) + COCO (variable)
