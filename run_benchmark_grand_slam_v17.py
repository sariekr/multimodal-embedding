...
def safe_get_text(item, col_name, dataset_name="", run_seed=0):
    ...
    if isinstance(val, list): 
        if len(val) == 0: return ""
        return str(val[0]) # Standardize first caption
...
if __name__ == "__main__":
    ...
    # --- OFFICIAL DOCVQA CHANGE ---
    try: 
        print("Loading DocVQA (lmms-lab/DocVQA)...")
        # Try 'test' split first, if not 'validation'
        try:
            raw_docvqa = load_dataset("lmms-lab/DocVQA", split="test")
        except:
            print("  'test' split not found, trying 'validation'...")
            raw_docvqa = load_dataset("lmms-lab/DocVQA", split="validation")
    ...
    print("\n" + "="*120)
    print("GRAND SLAM BENCHMARK RESULTS (V17 - OFFICIAL DATASET)")
    print("="*120)
