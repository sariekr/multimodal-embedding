...
MODELS_TO_TEST = [
    ...
    # 3. SigLIP Large (The Problem Child - Fixed Type)
    {"name": "SigLIP-Large",  "id": "google/siglip-large-patch16-384", "type": "siglip", "batch_size": 32},
    ...
    # 8. DataComp XL (The Missing One - Fixed Loading)
...
if __name__ == "__main__":
    ...
    for m_info in MODELS_TO_TEST:
        ...
        try:
            ...
            if m_info["type"] == "colpali":
                ...
                # Force Siglip classes for stability
                ...
                try:
                    ...
                    print("    Safetensors failed, attempting fallback (weights_only=False might be needed)...")
                    # If it fails, try without safetensors (might need manual approval but we try standard)
            ...
            # Add empty row to show error in table
    ...
    print("\n" + "="*120)
    print("GRAND SLAM BENCHMARK RESULTS (V12 - THE PERFECT STORM)")
    print("="*120)
    if len(results) > 0:
        keys = results[0].keys()
        ...
        pd.DataFrame(results).to_csv("benchmark_v12_results.csv", index=False)
