...
MODELS_TO_TEST = [
    ...
    # NEW CHALLENGER: VLM2Vec (LLM based embedding)
    {"name": "VLM2Vec-Full",  "id": "TIGER-Lab/VLM2Vec-Full", "type": "dense", "trust": True, "batch_size": 8}, # Heavy model
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(queries), bs):
                ...
                if model_info["type"] == "colpali":
                    ...
                    # VLM2Vec might need specific prompt templates, but AutoProcessor handles standard input
...
if __name__ == "__main__":
    ...
    print("\n" + "="*120)
    print("GRAND SLAM BENCHMARK RESULTS (V14 - NEW CHALLENGER VLM2Vec)")
    print("="*120)
    if len(results) > 0:
        keys = results[0].keys()
        ...
        pd.DataFrame(results).to_csv("benchmark_v14_results.csv", index=False)
