...
def compute_bidirectional_metrics(txt_embeds, img_embeds, scores_t2i, scores_i2t,
    ...
    Args:
        txt_embeds: Text embeddings [N_text, D]
        img_embeds: Image embeddings [N_img, D]
...
def run_retrieval_benchmark(model, processor, model_info, dataset, dataset_name, text_col="caption", image_col="image", use_all_captions=False):
    ...
    try:
        if use_all_captions:
            # Flickr30k standard: N unique images, M captions (M = N * captions_per_image)
            # Gallery: Unique images only
            # Queries: All captions
            images = []
            queries = []
