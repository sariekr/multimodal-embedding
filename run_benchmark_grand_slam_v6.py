...
MODELS_TO_TEST = [
    ...
    # 4. OpenAI CLIP (Fix: Force safetensors if avail, or catch error)
    {"name": "OpenAI-CLIP-L", "id": "openai/clip-vit-large-patch14-336", "type": "dense", "batch_size": 32},

    # 5. BGE-Visualized (Re-added with trust_remote_code)
    {"name": "BGE-Visualized", "id": "BAAI/bge-visualized-base-en-v1.5", "type": "dense", "trust": True, "batch_size": 32},

    # 6. EVA02-CLIP-L (Re-added with more common HF ID)
    {"name": "EVA02-CLIP-L", "id": "BAAI/EVA02-CLIP-L-14-336", "type": "dense", "trust": True, "batch_size": 32}
]
...
def run_winoground_safe(model, processor, model_info):
    ...
    try:
        ...
        print("    Winoground standard load failed. Skipping to avoid crash.")
...
if __name__ == "__main__":
        ...
        try:
            ...
            if m_info["type"] == "colpali":
                ...
                try:
                    ...
                    print(f"    Safetensors failed for {m_name}: {safetensors_e}. Trying standard load...")
    ...
    print("\n" + "="*80)
    print("GRAND SLAM BENCHMARK RESULTS (V6 - EXPANDED & ROBUST)")
    print("="*80)
