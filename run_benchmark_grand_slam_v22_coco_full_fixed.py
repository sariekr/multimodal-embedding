"""
V22 - MS-COCO (Full 5k) & Winoground - FIXED

CHANGES from V21:
1. 🔴 FIXED ColPali I2T: Now correctly re-computes scores (MaxSim is asymmetric).
2. 🎯 RESTORED Winoground: Added back compositional reasoning benchmark.
3. ✅ STRICT VALIDATION: 
   - Assert 5000 images.
   - Assert 5 captions/image.
   - Fail if >1% download error.
4. 📊 IMPROVED METRICS: 
   - Added SigLIP-Base & LAION baselines.
   - Added tqdm progress bars.
   - Reporting R@10.
5. ⚖️ STANDARDIZED: Attempting BS=32 for MetaCLIP (monitor for OOM).
...
def prepare_dataset(ds):
    ...
    if len(valid_indices) < 4950: # <1% failure allowed
...
def run_benchmark_coco(model, processor, m_info, dataset, valid_indices):
    ...
    try:
        with torch.no_grad():
            ...
            # Scoring
            if m_info["type"] == "colpali":
                # T2I: Text Queries -> Image Docs
                print("    > Computing ColPali T2I Scores...")
                scores_t2i_list = []
                for i in range(0, len(txt_embeds), 10):
                    q_chunk = [t.to(DEVICE) for t in txt_embeds[i:i+10]]
                    scores_row = []
                    for j in range(0, len(img_embeds), 10):
                ...
                # I2T: Image Queries -> Text Docs (RE-COMPUTE, DO NOT TRANSPOSE)
                print("    > Computing ColPali I2T Scores (Re-computing)...")
                scores_i2t_list = []
                for i in range(0, len(img_embeds), 10):
                    q_chunk = [img.to(DEVICE) for img in img_embeds[i:i+10]] # Images as queries
                    scores_row = []
                    for j in range(0, len(txt_embeds), 10):
                        d_chunk = [t.to(DEVICE) for t in txt_embeds[j:j+10]] # Text as docs
                        s = processor.score(q_chunk, d_chunk) # Flip inputs!
                        scores_row.append(s.cpu())
...
if __name__ == "__main__":
    ...
    df.to_csv("benchmark_v22_coco_full.csv", index=False)
    print("\n✅ Saved to benchmark_v22_coco_full.csv")
