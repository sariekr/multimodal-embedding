...
# --- MODEL LIST (Proven working V15 list) ---
MODELS_TO_TEST = [
...
if __name__ == "__main__":
    print(">>> LOADING DATASETS (Safe Replacements)...")
    ...
    # Use 'train' split for DocVQA (has 1000 samples), 'test' only has 200
    try: 
        print("Loading DocVQA (nielsr/docvqa_1200_examples)...")
        raw_docvqa = load_dataset("nielsr/docvqa_1200_examples", split="train") # Train split has 1000
        print(f"DocVQA loaded: {len(raw_docvqa)} samples")
    except Exception as e:
        print(f"DocVQA failed: {e}"); raw_docvqa = None

    # Use DiffusionDB as stable COCO replacement (streaming)
    try: 
        print("Loading DiffusionDB (COCO alternative)...")
        # Stream first 5000 to allow shuffling
        ds_diff_stream = load_dataset("poloclub/diffusiondb", "2m_first_1k", split="train", streaming=True)
        raw_diffdb = list(ds_diff_stream.take(SAMPLE_SIZE * 2)) # Take buffer
        print(f"DiffusionDB loaded: {len(raw_diffdb)} samples")
    except Exception as e:
        print(f"DiffusionDB failed: {e}"); raw_diffdb = None
    ...
    for m_info in MODELS_TO_TEST:
        ...
        model_runs = {"Flickr": [], "DocVQA": [], "DiffDB": [], "Winoground": []}
        ...
        for run_idx in range(NUM_RUNS):
            ...
            if raw_diffdb:
                # Manual shuffle for list
                random.shuffle(raw_diffdb)
                ds = raw_diffdb[:SAMPLE_SIZE]
                res = run_retrieval_benchmark(model, processor, m_info, ds, "DiffDB", "prompt", "image", run_idx)
                if res: model_runs["DiffDB"].append(res)

            if raw_docvqa:
                ds = raw_docvqa.shuffle(seed=current_seed).select(range(SAMPLE_SIZE))
        ...
        for dataset_name, runs in model_runs.items():
            ...
            for k in metric_keys:
                ...
                elif "Embed" in k: agg_row[f"Embed Size"] = values[0]
    ...
    print("\n" + "="*120)
    print("GRAND SLAM BENCHMARK RESULTS (V20 - SAFE & STATISTICAL)")
    print("="*120)
    
    df = pd.DataFrame(final_agg_results)
    cols = ["Model", "Embed Size"]
    for ds in ["Flickr", "DiffDB", "DocVQA"]: cols.extend([f"{ds} R@1", f"{ds} R@5", f"{ds} Time"])
    cols.append("Winoground Wino Group")
    final_cols = [c for c in cols if c in df.columns]
    print(df[final_cols].to_markdown(index=False))
    df.to_csv("benchmark_v20_results.csv", index=False)
